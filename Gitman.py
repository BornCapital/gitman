...
def setacl(file, mode, xattrs):
  ...
  acl2 = posix_acl.ACL(text=xattrs)
  for e in acl2:
...
class ConcatCrontabs:
  def __init__(self, files):
    ...
    print('tmpfilename=', self.tmpfilename)
...
class GitMan:
  ...
  def setperms(self, file, args):
    ...
    if args['extended_attrs']:
      setacl(file, int(args['mode'], 8), args['extended_attrs'])
